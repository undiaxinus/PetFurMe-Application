...
const AddAppointment = ({ route, navigation }) => {
  ...
  const fetchUserPets = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.0.110/PetFurMe-Application/api/pets/get_user_pets.php?user_id=${user_id}`);
  ...
  const handleSaveAppointment = async () => {
    if (
      ...
      try {
        ...
        const response = await fetch('http://192.168.0.110/PetFurMe-Application/api/appointments/save.php', {
          method: 'POST',
          headers: {
  ...
  const checkAvailability = async (date) => {
    try {
      ...
      const response = await fetch(
        `http://192.168.0.110/PetFurMe-Application/api/appointments/check_availability.php?date=${formattedDate}`
      );
      const result = await response.json();
